...
#' @export
#'
multita <- function(dat, npc, met=c(1,2), sca =TRUE, logt=FALSE, detrend=FALSE){
  multimvar <- list()
  cptime <- c()
  for (i in met){
    strt <- Sys.time()
    newmvar <- ita(dat, npc, met=i, sca = sca, logt=logt, detrend = detrend)
    ...
    if (!is.null(newmvar$ts)){
      for (i in 1:npc){
        if(cor(multimvar[[1]]$ts[,i], newmvar$ts[,i])<0){
